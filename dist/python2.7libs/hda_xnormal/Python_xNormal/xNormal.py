...
def run_config(conf):
    ...
    f = open(_config_folder, 'w')
    f.write(conf)
    f.close()

    retcode = os.system("\"%s\" %s" % (path, _config_folder))
    os.remove(_config_folder)
...
def config(high_meshes, low_meshes, opts):
    ...
    color_opt_string = "\n    ".join(
        ['<%s R="%i" G="%i" B="%i" />' % (o[0], o[1][0], o[1][1], o[1][2]) for o in color_opts])
    conf_string += "    %s\n" % color_opt_string
